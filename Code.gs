...
const CONFIG={
  ...
  SHEET_NAME:"Worldwide Remote",
  CONTACTS_SHEET:"Morocco Contacts",
...
function getSheet_(name){
  const ss=getSS_(), n=String(name||CONFIG.SHEET_NAME).trim()||CONFIG.SHEET_NAME;
...
function getContacts_(){
  const ss=getSS_();let sh=ss.getSheetByName(CONFIG.CONTACTS_SHEET);
  if(!sh)sh=ss.insertSheet(CONFIG.CONTACTS_SHEET);
  sh.getRange(1,1,1,7).setValues([["Date Found","Role","Company","Location","Email","Email Type","Company Website"]]);sh.setFrozenRows(1);return sh;
}
...
function doPost(e){
  try{
    ...
    if(b.mode==="pending")return getPending_(b.limit||200,b.sheet||"ALL");
...
function doGet(e){
  try{
    ...
    return json_({status:"ok",service:"jobs-webhook",time:new Date().toISOString()});
  }catch(err){return json_({status:"error",message:String(err)});}
}
function getPending_(limit,sheetName){
  const names=sheetName&&sheetName!=="ALL"?[sheetName]:CONFIG.SHEETS;
  const out=[],max=Math.min(Number(limit)||200,500),cutoff=Date.now()-CONFIG.MAX_DEEP_AGE_DAYS*86400000;
  for(const name of names){
    const sh=getSheet_(name),v=sh.getDataRange().getValues();
    for(let r=1;r<v.length&&out.length<max;r++){
      const row=v[r],deep=String(row[COL.DEEP_STATUS-1]||"").toUpperCase(),id=String(row[COL.ID-1]||"").trim();
      if(!id||(deep&&deep!=="PENDING"&&deep!=="ERROR"))continue;
      const d=new Date(row[COL.DATE-1]).getTime();if(d&&d<cutoff)continue;
      out.push({id,entreprise:row[COL.COMPANY-1],intitule:row[COL.TITLE-1],lien:row[COL.LINK-1],company_site:row[COL.COMPANY_SITE-1],deep_status:deep,sheet:name});
    }
  }
  return json_({status:"success",jobs:out,count:out.length});
}
function addJobs_(jobs,sheetName){
  ...
  for(const j of jobs){
    ...
    if(!title||!company||company==="Unknown")continue;
    rows.push([j.date_detection||new Date().toISOString(),"NEW",j.role_cible||title,title,company,j.lieu||"Casablanca",j.remote===true?"YES":"NO",
      j.source||"",j.lien||"",id,j.company_site||"",j.emails_rh||"",j.deep_status||"PENDING",j.fit_score||"",j.fit_reasons||"",j.salary||"",
      j.description||"",new Date(),j.posted_age||"",j.posted_within_24h||"UNKNOWN",j.search_type||"",j.email_status||"PENDING",
      j.email_source||"",j.spontaneous||"NO"]);
    ids.add(id);
  }
...
function enrichJobs_(updates,sheetName){
  const bySheet={};for(const u of updates){const n=u.sheet||sheetName||CONFIG.SHEET_NAME;(bySheet[n]||(bySheet[n]=[])).push(u);}
  let updated=0;
  for(const n in bySheet){
    const sh=getSheet_(n),v=sh.getDataRange().getValues(),map=new Map();
    for(let r=1;r<v.length;r++){const id=String(v[r][COL.ID-1]||"").trim();if(id)map.set(id,r+1);}
    for(const u of bySheet[n]){
      const row=map.get(String(u.id||"").trim());if(!row)continue;
  ...
  return json_({status:"success",updated,received:updates.length});
}
function addContacts_(contacts){
  const sh=getContacts_(),v=sh.getDataRange().getValues();
  const seen=new Set(v.slice(1).map(r=>String(r[4]||"").toLowerCase().trim()).filter(Boolean)),rows=[];
  for(const c of contacts){
    const email=String(c.email||"").toLowerCase().trim();if(!email||seen.has(email)||!email.includes("@"))continue;
    rows.push([c.date_found||new Date().toISOString(),c.role||"",c.company||"",c.location||"",email,c.email_type||"Public company contact",c.company_website||""]);seen.add(email);
  }
  if(rows.length)sh.getRange(sh.getLastRow()+1,1,rows.length,7).setValues(rows);
  return json_({status:"success",added:rows.length,received:contacts.length,sheet:CONFIG.CONTACTS_SHEET});
}
function json_(obj){return ContentService.createTextOutput(JSON.stringify(obj)).setMimeType(ContentService.MimeType.JSON);}
function setupRemoteSheet(){CONFIG.SHEETS.forEach(getSheet_);}
function setupMoroccoSheet(){getSheet_("Morocco Remote");}
function setupMoroccoContacts(){getContacts_();}
