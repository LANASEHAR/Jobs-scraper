...
function getApplications_(limit,minFit){
  const out=[],max=Math.min(Number(limit)||5000,5000),threshold=Number(minFit)||65;
  for(const name of CONFIG.SHEETS){
    ...
    for(let r=1;r<v.length&&out.length<max;r++){
      const row=v[r],id=String(row[COL.ID-1]||"").trim();
      const status=String(row[COL.EMAIL_STATUS-1]||"").trim().toUpperCase();
      const fit=Number(row[COL.FIT_SCORE-1]||0);
      const email=String(row[COL.EMAILS-1]||"").trim();
      if(!id||!email||status==="SENT"||status==="SKIPPED"||fit<threshold)continue;
      const posted24=String(row[COL.POSTED_24H-1]||"").toUpperCase();
      if(posted24==="NO")continue;
      out.push({id, sheet:name, role_cible:row[COL.ROLE-1], intitule:row[COL.TITLE-1], entreprise:row[COL.COMPANY-1],
        lieu:row[COL.LOCATION-1], remote:row[COL.REMOTE-1], source:row[COL.SOURCE-1], lien:row[COL.LINK-1],
        emails_rh:email, fit_score:fit, fit_reasons:row[COL.FIT_REASONS-1], description:row[COL.DESCRIPTION-1],
        posted_age:row[COL.POSTED_AGE-1], posted_within_24h:posted24, company_site:row[COL.COMPANY_SITE-1]});
    }
  }
  return json_({status:"success",jobs:out,count:out.length,min_fit_score:threshold});
}
...
/* === Application sending moved to GitHub Actions ===
   GitHub Actions now handles matching, CV attachment, SMTP delivery and SENT
   status updates at the configured 08:05 and 09:05 Casablanca windows.
   Keep these compatibility functions so an old Apps Script trigger cannot
   accidentally send duplicate applications.
*/
function sendMorningEmails(){
  return;
}
function setupHourlyMorningEmailSender(){
  Logger.log("Application sending is disabled in Apps Script. GitHub Actions is the sender.");
}
