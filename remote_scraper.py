...
ROLES = [
    ...
    "Account Manager", "Customer Account Manager",
    ...
    "Operations Coordinator", "Business Operations Specialist",
    "Administrative Coordinator", "Sales Operations Specialist",
    "Commercial Operations Specialist", "E-commerce Specialist",
    "Shopify Specialist", "CRM Specialist", "Back Office Specialist",
    "Project Coordinator", "Designer"
]
EXCLUDE = [
    "senior", "sr.", "sr ", "lead", "principal", "director", "vp ",
    "vice president", "head of", "chief", "staff", "architect",
    "internship", "intern ", "doctor", "nurse", "software engineer",
    "developer", "data scientist", "machine learning", "devops",
...
PATHS = [
    ...
    "/join-us", "/work-with-us", "/recruitment", "/human-resources",
    "/hr", "/about", "/en/contact", "/en/careers", "/fr/contact"
]
...
def age_hours(t):
    t = clean(t).lower()
    if any(x in t for x in ["just posted", "today", "il y a quelques", "il y a 1 heure"]):
        return 0
...
def jid(source, url, title):
    return "job_" + hashlib.sha256((source + "|" + url + "|" + title).encode()).hexdigest()[:18]

def job(title, company, loc, remote, source, url, age="", desc="", kind=None):
    return {
        "id": jid(source, url, title), "date_detection": now(), "statut": "NEW",
        "role_cible": title, "intitule": title, "entreprise": company or "Unknown",
        "lieu": loc or ("Remote / Worldwide" if remote else "Casablanca"),
        "remote": bool(remote), "source": source, "lien": url,
        "company_site": "", "emails_rh": "", "deep_status": "PENDING",
        "fit_score": "", "fit_reasons": "", "salary": "", "description": clean(desc),
        "posted_age": clean(age),
        "posted_within_24h": "YES" if fresh(age) else "UNKNOWN",
...
def parse_linkedin(html, remote, search_type):
    ...
    for card in soup.select("li.base-card, li.jobs-search__results-list, div.base-card"):
        ...
        if title and company and target(title) and fresh(age):
            out.append(job(title, company, loc, remote, "LinkedIn",
                           urljoin("https://www.linkedin.com", href), age, kind=search_type))
    return out
...
def linkedin_guest_search(keywords, location, remote, search_type):
    ...
    h = fetch("https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search" + params, 15, 2)
...
def results(q, limit=8):
    ...
    for a in BeautifulSoup(h, "html.parser").select("a.result__a")[:limit]:
        ...
        if u.startswith("http"):
            out.append((clean(a.get_text(" ", strip=True)), u))
    return out
...
def linkedin_web_fallback(keywords, location, remote, search_type):
    ...
    for title, url in results(q, 10):
        ...
        # Search fallback is only accepted when the result itself contains a recent-time cue.
        snippet = title.lower()
        if not any(x in snippet for x in ["hour", "today", "minute"]):
            continue
        out.append(job(keywords, "", location, remote, "LinkedIn Search", url, "today", title, search_type))
    return out
...
def parse_indeed(html, kind, remote):
    ...
    for card in soup.select("div.job_seen_beacon, div.cardOutline, td.resultContent"):
        ...
        if not title or not target(title, desc) or not age or not fresh(age) or url in seen:
            continue
        seen.add(url)
        out.append(job(title, clean(ce.get_text(" ", strip=True) if ce else ""),
                       clean(le.get_text(" ", strip=True) if le else ""),
    ...
    for q in ROLES:
        for kind, loc, remote in targets:
            ...
            for j in parse_indeed(h, kind, remote):
                if j["id"] not in seen:
                    seen.add(j["id"]); out.append(j)
    print(f"[Indeed] total={len(out)}", flush=True)
    return out
...
def company_site(company):
    ...
    for q in [f'"{company}" official website', f'"{company}" careers jobs', f'"{company}" recruitment Casablanca']:
        for _, u in results(q):
            ...
            if host and not any(host == b or host.endswith("." + b) for b in BLOCKED):
                return "https://" + host
    return None
...
def extract_emails(html):
    ...
    return {
        e.lower().strip(" .;,<>\"'")
        for e in found
        if "@" in e and e.lower().split("@")[-1] not in BAD
...
def enrich(j):
    ...
    if not site:
        return {"id": j["id"], "sheet": j.get("sheet"), "company_site": "",
                "emails_rh": "", "deep_status": "NO_SITE", "email_status": "NOT_FOUND", "email_source": ""}
    p = urlparse(site); base = f"{p.scheme}://{p.netloc}"; domain = p.netloc.lower().replace("www.", "")
    es, pages = set(), False
    for path in PATHS:
        h = fetch(base + path, 10, 1)
        if h:
            pages = True; es |= extract_emails(h)
    for q in [f'"{j.get("entreprise","")}" recruitment email',
              f'"{j.get("entreprise","")}" careers email',
    ...
    return {"id": j["id"], "sheet": j.get("sheet"), "company_site": site,
...
def spontaneous():
    out, seen = [], set()
    for q in ROLES:
        print(f"[Spontaneous] Designer/company search: {q}", flush=True)
        for title, u in results(f'"{q}" Casablanca careers jobs recrutement', 10):
            ...
            if (not host or any(host == b or host.endswith("." + b) for b in BLOCKED)
                or not any(k in low for k in ("career", "careers", "jobs", "recruit", "emploi", "job"))):
                continue
            j = job(q + " — candidature spontanée", title or host, "Casablanca", False,
                    "Company Website Search", u, "DIRECT", "", "CASABLANCA_ONSITE")
            j["spontaneous"] = "YES"
            if j["id"] not in seen:
                seen.add(j["id"]); out.append(j)
    print(f"[Spontaneous] total={len(out)}", flush=True)
    return out
...
def post(payload):
    ...
    for i in range(3):
        try:
            ...
            r.raise_for_status()
            print(f"[Sheet] {r.status_code} {r.text[:300]}", flush=True)
            return True
        except requests.RequestException as e:
            print(f"[Sheet] attempt {i+1}: {e}", flush=True)
            time.sleep(2 ** i)
    return False

def pending():
    try:
        r = requests.get(WEBHOOK, params={"action": "pending", "limit": 200}, timeout=(10, 60))
        r.raise_for_status()
        return r.json().get("jobs", [])
    except Exception as e:
        print(f"[Pending GET] {e}", flush=True)
        try:
            r = requests.post(WEBHOOK, json={"mode": "pending", "limit": 200}, timeout=(10, 60))
            r.raise_for_status()
            return r.json().get("jobs", [])
        except Exception as e2:
            print(f"[Pending POST] {e2}", flush=True)
            return []

def deep(limit):
    js = pending()[:limit]
    print(f"[DEEP] pending={len(js)} limit={limit}", flush=True)
    updates = []
    for i, j in enumerate(js, 1):
        ...
        try:
            ...
            updates.append({"id": j.get("id"), "sheet": j.get("sheet"),
                            "deep_status": "ERROR", "email_status": "ERROR",
                            "deep_error": str(e)[:250]})
        if len(updates) >= 10:
...
def scrape():
    ...
    alljobs = linkedin() + indeed() + spontaneous()
    seen, unique = set(), []
    for j in alljobs:
        if j["id"] not in seen:
