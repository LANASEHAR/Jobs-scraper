...
EXCLUDE = [
    "senior","sr.","sr ","lead","principal","director","vp ","vice president","head of ",
...
def age_hours(t):
    t = clean(t).lower()
    if any(x in t for x in ["just posted","today","il y a quelques","il y a 1 heure","new"]):
        return 0
...
def jid(source,url,title):
    # URL-first identity prevents the same offer being duplicated across sources.
...
def linkedin_guest_search(keywords,location,remote,search_type):
    # One page per query is deliberate: repeated pagination was triggering LinkedIn 429s.
...
def results(q,limit=10):
    ...
    if not h:
        return []
    return [
        (clean(a.get_text(" ",strip=True)),a.get("href",""))
...
def linkedin_web_fallback(keywords,location,remote,search_type):
    queries = (
        ...
        if remote else
        [f'site:linkedin.com/jobs/view "{keywords}" Casablanca']
    )
    out = []
    seen = set()
    for q in queries:
...
def linkedin():
    ...
    for q in ROLES:
        for st,loc,remote in targets:
            ...
            # Always supplement with public search when the guest endpoint is blocked
            # or returns too little data.
            if len(found) < 5:
...
def enrich(j):
    ...
    for q in [
        f'"{j.get("entreprise","")}" recruitment email',
...
def spontaneous():
    ...
    for q in ROLES:
        ...
        query = f'"{q}" Casablanca careers jobs recrutement'
        for title,u in results(query,15):
            host = urlparse(u).netloc.lower().replace("www.","")
            low = (title + " " + u).lower()
            if (not host or any(host == b or host.endswith("." + b) for b in BLOCKED)
                ...
                continue
            j = job(
                q + " — candidature spontanée",
                title or host,
                "Casablanca",False,"Company Website Search",u,
                "DIRECT","", "CASABLANCA_ONSITE"
            )
            j["spontaneous"] = "YES"
            if j["id"] not in seen:
                seen.add(j["id"]); out.append(j)
...
def post_jobs(jobs, sheet):
    ...
    for j in jobs:
        j["sheet"] = sheet
    # Small batches mean offers reach the Sheet during the run, not only at the end.
...
def deep(limit):
    ...
    for i,j in enumerate(js,1):
        ...
        try:
            ...
            updates.append({
                "id":j.get("id"),"sheet":j.get("sheet"),
                "deep_status":"ERROR","email_status":"ERROR","deep_error":str(e)[:250]
            })
        if len(updates) >= 10:
...
def scrape():
    ...
    seen = set()
    totals = {v:0 for v in aliases.values()}

    # Stream each source to the Sheet as soon as it completes.
    for source_name, fn in [
        ...
        for sheet, batch in groups.items():
            if batch:
                post_jobs(batch,sheet)
                totals[sheet] += len(batch)
